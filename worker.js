...
function generateApiKey(name) {
	...
	const raw = `nova_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
...
function obfuscateCode(srcText) {
	...
	cleanCode = cleanCode.replace(/export\s+default\s+/g, 'const _0xNovaModule = ');
	cleanCode += '\nreturn _0xNovaModule;';
...
log(`[Sub.NodeLimit] seed=${_perUserSeed || '(none)'} hasOwnCleanIp=${_hasOwnCleanIp} beforeLimit=${ipNivcharMale.length} cap=${_nodeCap}`);
						...
						if (sugMinuy === 'singbox') {
							...
							if (_fakeLines.length) tochenMinuy = novaInjectFakeSingbox(tochenMinuy, _fakeLines, config_JSON.UUID, host);
							responseHeaders["content-type"] = 'application/json; charset=utf-8';
						} else if (sugMinuy === 'clash') {
							tochenMinuy = tikunChamClashMinuy(tochenMinuy, config_JSON, hagdarotReshet, warpAccount);
							if (_fakeLines.length) tochenMinuy = novaInjectFakeClash(tochenMinuy, _fakeLines, config_JSON.UUID, host);
						...
						if (_fakeLines.length && sugMinuy === 'mixed') {
							const _fakeUris = novaFakeVmessUris(_fakeLines, config_JSON.UUID, host);
...
function _warpPublicView(a, epOverride) {
	...
	if (a.privateKey && a.peerPublicKey) {
		...
		v.node = `${_D_._wg_}://${encodeURIComponent(a.privateKey)}@${epFull}/?publickey=${encodeURIComponent(a.peerPublicKey)}${reservedStr}&address=${encodeURIComponent(addr)}&mtu=1280#Nova-WARP`;
...
async function buildRegisteredWarpNode(env) {
	...
	return `${_D_._wg_}://${encPriv}@${ep.includes(':') ? ep : ep + ':2408'}/?publickey=${encPub}${reservedStr}&address=${addr}&mtu=1280#Nova-WARP`;
}
...
// ===== CUSTOM AUTO-SETUP (Nova fork) =====
function _randPass(len = 16) {
	const chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
	const bytes = crypto.getRandomValues(new Uint8Array(len));
	return Array.from(bytes, b => chars[b % chars.length]).join('');
}
function _randHex(n = 8) {
	return Array.from(crypto.getRandomValues(new Uint8Array(n)), b => b.toString(16).padStart(2, '0')).join('');
}

/**
 * اگر هنوز admin_pass ست نشده:
 *  - رمز ادمین رندوم می‌سازد و در KV می‌نویسد
 *  - multiUser را روشن می‌کند
 *  - یک یوزر پیش‌فرض با حجم نامحدود و connLimit=3 می‌سازد
 * خروجی: { password, user, subPath } یا null اگر از قبل setup شده
 */
async function ensureCustomAutoSetup(env, host) {
	if (!env.KV || typeof env.KV.get !== 'function') return null;

	// اگر قبلاً setup شده، کاری نکن
	const existing = await env.KV.get('admin_pass');
	if (existing) return null;

	// قفل ساده تا دو درخواست همزمان دو بار setup نکنند
	const lock = await env.KV.get('custom_setup_done');
	if (lock) return null;
...
async function tipulAshafHatkana(request, env, url, adminPassword, encryptionKey, UA) {
  ...
  if (hasStore && !adminPassword) {
		const setup = await ensureCustomAutoSetup(env, url.host);
		if (setup) {
			adminPassword = setup.password;
			// برای /install/status و UI: دیگر «نیاز به ست کردن رمز» نیست
		}
	}
	// اگر فقط می‌خواهی info را ببینی:
	if (sub === 'credentials') {
		// فقط یک‌بار بعد از setup قابل خواندن — اختیاری، بعداً می‌توانی محدودترش کنی
		const info = hasStore ? await env.KV.get('custom_setup_info') : null;
		if (!info) return new Response(JSON.stringify({ error: 'not_ready' }), { status: 404, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
		return new Response(info, { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8', 'Cache-Control': 'no-store' } });
	}
	// --- END CUSTOM ---
	if (sub === 'status') {
